...
//        $this->set_js_lib($this->default_javascript_path . '/jquery_plugins/chosen.proto.min.js');
    ...
    protected function load_js_jqueryui() {
        $this->set_css($this->default_css_path . '/ui/start/' . grocery_CRUD::JQUERY_UI_CSS);
...
//            unset($js_files[sha1($this->default_javascript_path . '/jquery_plugins/ui/' . grocery_CRUD::JQUERY_UI_JS)]);
            unset($css_files[sha1($this->default_css_path . '/ui/start/' . grocery_CRUD::JQUERY_UI_CSS)]);
            unset($js_files[sha1($this->default_javascript_path . '/jquery_plugins/ui/' . grocery_CRUD::JQUERY_UI_JS)]);
        }
    ...
    protected function get_dropdownadd_input($field_info, $value) {
        $this->load_js_chosen();
        $this->set_js_config($this->default_javascript_path . '/jquery_plugins/config/jquery.chosen.config.js');

        $select_title = str_replace('{field_display_as}', $field_info->display_as, $this->l('set_relation_title'));

        $input = "<select id='field-{$field_info->name}' name='{$field_info->name}' class='chosen-select-add' data-placeholder='" . $select_title . "'>";
        $options = array('' => '') + $field_info->extras;
        foreach ($options as $option_value => $option_label) {
            $selected = !empty($value) && $value == $option_value ? "selected='selected'" : '';
            $input .= "<option value='$option_value' $selected >$option_label</option>";
    ...
    protected function get_multiselectadd_input($field_info, $value) {
        $this->load_js_chosen();
        $this->set_js_config($this->default_javascript_path . '/jquery_plugins/config/jquery.chosen.config.js');
        ...
        $select_title = str_replace('{field_display_as}', $field_info->display_as, $this->l('set_relation_title'));
        $input = "<select id='field-{$field_info->name}' name='{$field_info->name}[]' multiple='multiple' size='8' class='chosen-multiple-select-add' data-placeholder='$select_title' style='width:510px;' >";
    ...
    protected function get_relation_n_n_input($field_info_type, $selected_values) {
        ...
        if ($has_priority_field || $is_ie_7) {
            $this->set_css($this->default_css_path . '/ui-custom/' . grocery_CRUD::JQUERY_UI_CSS);
...
//    const JQUERY_UI_JS = "jquery-ui-1.10.3.custom.min.js";
    const JQUERY_UI_JS = "jquery-ui-1.11.1.min.js";
